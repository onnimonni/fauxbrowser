...
- **Verified against shop.example.com** (Cloudflare IUAM + UA-based client
...
- **Browser-plausible default headers** (User-Agent, Accept,
  ...
  doesn't set them, so sites like `shop.example.com` that check for
...
straight through. Verified against shop.example.com (Cloudflare IUAM +
